...
export const getProducts = params => {
  return dispatch => {
    return axios.get('/apps/subscription/products', { params }).then(res => {
...
export const addToCart = id => {
  return (dispatch, getState) => {
    return axios.post('/apps/subscription/cart', { productId: id }).then(res => {
      dispatch({ type: 'ADD_TO_CART', data: res.data })
      dispatch(getProducts(getState().subscription.params))
    })
  }
}
...
export const getProduct = slug => {
  return dispatch => {
    return axios.get(`/apps/subscription/products/${slug}`).then(res => {
...
export const getPlan = id => {
  return async dispatch => {
    await axios
      .get('/apps/subscription/plan', { id })
      .then(response => {
        dispatch({
          type: 'GET_PLAN',
          selectedPlan: response.data.plan
        })
